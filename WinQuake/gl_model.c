...
void Mod_LoadTextures (lump_t *l)
{
	...
	for (i=0 ; i<m->nummiptex ; i++)
	{
		...
		else
		{
			...
			tx->gl_texturenum = GL_LoadTexture (mt->name, tx->width, tx->height, (byte *)(tx+1), true, false, TEX_TYPE_MAP);
...
void Mod_LoadFaces (lump_t *l, int bsp2)
{
	...
	for ( surfnum=0 ; surfnum<count ; surfnum++, out++)
	{
		if (bsp2)
		{
			...
			out->texinfo = loadmodel->texinfo + LittleLong(inl->texinfo);
			// lighting info
			for (i = 0; i<MAXLIGHTMAPS; i++)
				out->styles[i] = inl->styles[i]; 
			i = LittleLong(inl->lightofs);
			if (i == -1)
				out->samples = NULL;
			else
		...
		else
		{
			...
			out->texinfo = loadmodel->texinfo + LittleShort(ins->texinfo);
			// lighting info
			for (i = 0; i<MAXLIGHTMAPS; i++)
				out->styles[i] = ins->styles[i];
			i = LittleLong(ins->lightofs);
			if (i == -1)
				out->samples = NULL;
			else
				out->samples = loadmodel->lightdata + i;
...
void *Mod_LoadAllSkins (int numskins, daliasskintype_t *pskintype)
{
	...
	//		}
			...
			pheader->gl_texturenum[i][3] =
				...
				pheader->skinheight, (byte *)(pskintype + 1), true, false, TEX_TYPE_NONE);
			...
			for (j=0 ; j<groupskins ; j++)
			{
					...
					pheader->gl_texturenum[i][j&3] = 
						GL_LoadTexture (name, pheader->skinwidth, 
						pheader->skinheight, (byte *)(pskintype), true, false, TEX_TYPE_NONE);
...
void * Mod_LoadSpriteFrame (void * pin, mspriteframe_t **ppframe, int framenum)
{
	...
	*ppframe = pspriteframe;
	...
	pspriteframe->gl_texturenum = GL_LoadTexture (name, width, height, (byte *)(pinframe + 1), true, true, TEX_TYPE_NONE);
