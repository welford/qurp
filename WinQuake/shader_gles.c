...
static const GLenum enumToGLShaderTypes[] = {GL_VERTEX_SHADER, GL_FRAGMENT_SHADER};
...
static const GLenum enumToGLProgramParams[] = {GL_DELETE_STATUS, GL_LINK_STATUS, GL_VALIDATE_STATUS, GL_INFO_LOG_LENGTH, GL_ATTACHED_SHADERS, GL_ACTIVE_ATTRIBUTES, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, GL_ACTIVE_UNIFORMS, GL_ACTIVE_UNIFORM_MAX_LENGTH};
...
//	Shader Object
//---------------------------------
unsigned char CreateShader(SShader* const pShader, const ShaderType type, const char** const pSrcArray, const unsigned int n)
{
...
unsigned char AddShaderToProgram(const SShaderProgram* const pProgram, const SShader*  pShader)
{
	if(pProgram->handle != 0)
	{
		...
		{	//should I just compile for them? seems trivial
			printf("compile shader before adding:\n");
		}
	}
	return 0;
...
unsigned char LinkShaderProgram(SShaderProgram* const pShader)
{
	...
	return 1;//(pShader->linked && pShader->validate);
}
...
int GetAttributeLocation(const SShaderProgram* const pProgram, const char* const name)
{
	return glGetAttribLocation(pProgram->handle, name);
}
